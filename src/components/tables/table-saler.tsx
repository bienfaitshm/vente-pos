...
import type { ColumnSalerType } from "./columns";
...
const DataTableSaler: FunctionComponent<DataTableSalerProps> = ({
  ...
  const columns = useMemo(() => getSalerColumns({ actions }), [actions]);

  const { searchField, table } = useHookTable<ColumnSalerType>({
    data,
    columns,
